describe('RT2 - Fluxo de Edição de Produto', () => {
  ...
  it('Deve editar um produto existente', () => {
    ...
    cy.get('[data-testid="produto-row"]').last().find('[data-testid="editar-produto-button"]').click();
    ...
    cy.get('[data-testid="produto-row"]').last().should('contain', 'Mouse Óptico Sem Fio');
    cy.get('[data-testid="produto-row"]').last().should('contain', 'R$ 120.00');
    cy.get('[data-testid="produto-row"]').last().should('not.contain', 'Mouse Antigo');
  });
});